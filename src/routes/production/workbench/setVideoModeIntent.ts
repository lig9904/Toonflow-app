...
import { saveVideoModeIntent, VideoModeResolutionError } from "@/services/videoModeResolution";
...
export default express.Router().post("/", async (req: Request, res: Response) => {
  try {
    ...
    return res.send(success(await saveVideoModeIntent(u.db, req.body, actor.id)));
