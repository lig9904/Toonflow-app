...
| 片段时长合理 | 严重 | 每条分镜对应独立生成片段，时长只按本镜统计且符合模型能力，不按分类标签累计；含台词镜时长 ≥ 台词字数÷语速（~4 字/秒）+停顿+1s 安全余量；无台词镜 ≤6s | — |
...
| 连贯优先/拆分粒度 | 中等 | 已保存分镜与用户指定镜数均保留，逐镜独立生成；连续性由镜头起止状态和后期剪辑完成；画面描述 字数在执行层上限（15~50 字）内 | — |
...
审核时分别核对分镜数、有分镜的独立生成片段数以及各镜时长。额外空白片段单列，历史合并片段及其历史视频单列，不把空白卡片数或整段旧视频数当作逐镜完成证据。
