...
add("POST", ["listPrompts", "promptDetail", "promptHistory", "resetPrompt", "restorePrompt", "previewPrompt"].map((name) => `/api/setting/promptManage/${name}`), { scope: "admin", roles: ["admin"] });
add("POST", ["/api/production/review/listImageReviews"], project("read", "projectId", [{ table: "o_script", field: "scriptId", optional: true }]));
add("POST", ["/api/production/review/reviewImage"], project("edit", "projectId", [{ table: "o_script", field: "scriptId", optional: true }]));
add("POST", ["/api/project/addProject"], { scope: "team", action: "edit" });
add("POST", ["", "/groups", "/assets", "/asset", "/group", "/getBindings"].map((suffix) => `/api/production/trustedAssets${suffix}`), project("read"));
add("POST", ["/setBindings", "/syncBindings"].map((suffix) => `/api/production/trustedAssets${suffix}`), project("edit"));
add("POST", ["/getGroupCreation", "/listGroupCreations", "/getUpload", "/listUploads"].map((suffix) => `/api/production/trustedAssets${suffix}`), project("read"));
add("POST", ["/createGroup", "/syncGroupCreation", "/startUpload", "/syncUpload"].map((suffix) => `/api/production/trustedAssets${suffix}`), project("edit"));
