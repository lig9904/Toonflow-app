...
资产引用映射[{"id":数字,"type":"资产类型","name":"资产名称","mediaType":"image|video|audio","referenceLabel":"@图片N|@视频N|@音频N"}]
```
...
> **引用编号由服务端预先计算**：必须原样使用「资产引用映射」中的 `referenceLabel`，不得自行重新判断媒体类型或重新编号。
...
读取所有 `<storyboardItem>` 的 `videoDesc`，将其拆为各 `序号N` 子镜头，结合资产信息，按 Seedance 2.0 文本多参语法，把全部镜头整合为**一个完整的视频提示词**（非逐条独立）。资产素材是唯一参考素材（无分镜参考素材）。
...
素材按媒体类型独立编号：视觉图片使用 `@图片N`，视频使用 `@视频N`，音频使用 `@音频N`。编号以「资产引用映射」中的 `referenceLabel` 为准，同一媒体类型按映射出现顺序从 1 递增。

### 主体定义与引用 `<主体N>` / `<场景N>` / `<道具N>`

- **第一段集中定义**：`将 {referenceLabel} 中的[2-3 个稳定特征] 定义为 <标签k>（名字）`，其中 `{referenceLabel}` 必须取自「资产引用映射」。角色用 `<主体k>`、场景用 `<场景j>`、道具用 `<道具i>`，三类标签编号各自从 1 递增。
- **正文全程使用标签**：镜头正文只用 `<主体k>` / `<场景j>` / `<道具i>` 指代；需强调绑定或防歧义时用 `<主体k>{referenceLabel}`。
- 场景标签 `<场景j>` 绑定的场景素材**自带光影**，正文据此引用场景即可，不另描述光影。

### 断句防歧义（强制）

裸用 `referenceLabel` 紧接动词或方位词时（如"@视频1跑向…"）易触发数字粘连歧义，应改为 `<主体N>{referenceLabel}`，或在引用标签后补名词隔断（如"@视频1 中的男子"）。
...
输入资产：
```
资产信息[26, role, 张振华], [27, audio, 张振华音色], [29, scene, 废弃地堡内部], [32, tool, 黑色金属箱]
资产引用映射[{"id":26,"type":"role","name":"张振华","mediaType":"image","referenceLabel":"@图片1"},{"id":27,"type":"audio","name":"张振华音色","mediaType":"audio","referenceLabel":"@音频1"},{"id":29,"type":"scene","name":"废弃地堡内部","mediaType":"image","referenceLabel":"@图片2"},{"id":32,"type":"tool","name":"黑色金属箱","mediaType":"image","referenceLabel":"@图片3"}]
```

| 输入项 | 素材编号 | 主体标签 |
|--------|----------|----------|
| [26, role, 张振华] | `@图片1` | `<主体1>`（张振华） |
| [27, audio, 张振华音色] | `@音频1` | 张振华的音色参考 |
...
**第一段：总体设定 + 主体定义**
```
将 {角色素材referenceLabel} 中的[2-3 个稳定特征] 定义为 <主体1>（{名}{，音色参考 @音频M}）；将 {场景素材referenceLabel} 中的[…] 定义为 <场景1>（{场景}）{；将 {道具素材referenceLabel} 中的[…] 定义为 <道具1>（{道具}）}。
```

> 本模式无分镜参考素材：第一段**不出现**任何「{referenceLabel} 作为 镜头K 构图参考」。
...
- **优先取 audio 资产**：当该角色挂有 audio（音频）资产时，音色直接引用——`音色：取自 @音频M（{可补简要音色特征}）`。
...
3. **分类型引用语法 + 先定义后正文**：图片用 `@图片N`、视频用 `@视频N`、音频用 `@音频N`，各类型独立编号；主体先定义 `<主体N>`/`<场景N>`/`<道具N>` 再于正文引用；audio 挂主体后作音色来源；第一段集中绑定全部主体，正文不再重复定义。
4. **Asset ID 屏蔽 + 断句防歧义**：正文不裸写 assetId；`referenceLabel` 紧接动词/方位词时改 `<主体N>{referenceLabel}` 或补名词隔断。
5. **全部无分镜参考素材**：`referenceLabel` 仅映射资产，第一段不声明构图参考，正文不得引用任何分镜参考素材，且**严禁虚构不存在的分镜引用**。
...
10. **光影沿用场景素材自带光影**：场景资产的 `referenceLabel`（`<场景N>`）已携带光影，模型据此推导明暗/色温/方向；正文与约束包均**不写**任何光影方向/色温/明暗/色调。唯一例外是第三段「整体美术调性」行引用的画风固有风格标签（属风格锚定）。
