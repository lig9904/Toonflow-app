...
export async function startManualPolish(req:Request,batch:boolean){
 ...
 const input=z.object({projectId:z.coerce.number().int().positive(),items:z.array(item).min(1).max(50),otherTextPrompt:z.string().max(10000).default(''),concurrentCount:z.number().int().min(1).max(20).default(1),idempotencyKey:z.string().min(8).max(100)}).parse(raw);
 ...
 if(old){const c=(old.intent as any)?.context;if(old.projectId!==input.projectId||c?.otherTextPrompt!==input.otherTextPrompt||(c?.concurrentCount??1)!==input.concurrentCount||JSON.stringify(c.items.map((i:any)=>({assetsId:i.assetsId,expectedVersion:i.expectedVersion})))!==JSON.stringify(input.items.map(i=>({assetsId:i.assetsId,expectedVersion:i.expectedVersion}))))throw new BuiltinRuntimeError('CONFLICT','同一请求已绑定不同素材');return {runId:old.id,reused:true,total:input.items.length};}
 ...
 const result=await runtime.create({agentType:'productionAgent',projectId:input.projectId,scriptId:null,requestedBy:actor,idempotencyKey:key,prompt:input.otherTextPrompt||'根据当前素材身份和项目风格生成素材提示词',intent:{phase:'polishAssets',context:{projectId:input.projectId,items,otherTextPrompt:input.otherTextPrompt,concurrentCount:input.concurrentCount}},limits:{maxModelCalls:input.items.length,maxToolSteps:Math.min(200,input.items.length*2+3),maxOutputTokens:64000,maxImageGenerations:0,maxVideoGenerations:0}});runtime.start();return {runId:result.run.id,reused:result.reused,total:items.length};
}
