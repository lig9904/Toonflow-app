...
生成图片或视频只能在对应 authorization 次数大于 0 时选择；未授权时先完成用户要求的文本准备。
...
用户要求“生成全部衍生资产”等完整素材生产且图片额度大于 0 时，应选择 deriveAssets 和 generateImages，先登记衍生描述再生成对应图片。明确要求“仅分析”“只生成描述”时只选择 deriveAssets。用户指定已有衍生素材出图时，选择其真实衍生 ID 和 generateImages，不必重新派生。
