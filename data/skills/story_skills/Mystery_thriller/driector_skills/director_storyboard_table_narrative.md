...
下文关于镜头整合、一镜到底或叙事段落的建议，仅适用于尚未定稿且未指定镜数的镜头设计，不是合并已存分镜或生成片段的指令。已有分镜、人工指定镜数与逐镜生成规则优先；相邻独立镜头可以在剪辑中形成连续段落。
适用边界：以下节奏、时长和切镜规则仅为通用建议；用户明确的镜数、台词、总时长和长镜头优先，不得因本手册擅自拆镜、减对白或增加人物。
