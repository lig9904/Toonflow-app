...
import { ensureVideoPromptJobSchema, videoPromptFailureView, promptJobSuperseded } from "@/services/videoPromptJobs";
...
export default router.post(
  ...
  async (req, res) => {
    ...
    const columns=["id","trackId","trackVersion","idempotencyKey","state","reason","failureStage","draftPrompt","reviewDraftPrompt","resultPrompt","sourceSnapshot"];
    const jobQuery = u.db("ext_video_prompt_jobs").where({ projectId, scriptId }).whereIn("trackId", trackIds);
    const latestJobs = await jobQuery.clone().distinctOn("trackId").orderBy("trackId").orderBy("createdAt", "desc").orderBy("id","desc").select(columns);
    const requestedJobs = jobIds.length ? await jobQuery.clone().whereIn("id",jobIds).orderBy("createdAt","desc").select(columns) : [];
    ...
    const selectedJobs = [...requestedJobs,...latestJobs];
    ...
    const promptList = await Promise.all(tracks.map(async (track) => {
      ...
      const superseded=promptJobSuperseded(job,track,versionByTrack.get(Number(track.id))??0);
      return job
        ? { promptFailure:superseded?null:videoPromptFailureView(job), videoList, selectVideoId:track.videoId, promptReview, id: Number(track.id), jobId: String(job.id), idempotencyKey: String(job.idempotencyKey), state: superseded?track.state:state(String(job.state)), reason: superseded?"":job.reason ?? "", prompt: track.prompt ?? "", version: versionByTrack.get(Number(track.id)) ?? 0 }
        : { ...track, promptFailure:null, videoList, selectVideoId:track.videoId, promptReview, version: versionByTrack.get(Number(track.id)) ?? 0 };
    }));
