...
import { readStructuredModelOutput } from "../../lib/structuredModelOutput";
...
export const configuredScriptModel: StructuredScriptModel = {
  async generate(request) {
    ...
    const result = await readStructuredModelOutput({ ...request, maxOutputTokens: limit ?? 0 }, (onFinish) => u.Ai.Text(request.role, thinking, request.thinkLevel).invoke({
