...
/** `GENERATED BY DEFAULT` keeps explicit legacy IDs valid while making new rows portable from SQLite rowid. */
const identityId = (table: Knex.CreateTableBuilder) => table.specificType("id", "bigint GENERATED BY DEFAULT AS IDENTITY").notNullable();
const bigint = (table: Knex.CreateTableBuilder, column: string) => table.bigInteger(column);
const baseIdentityTables = [
  "o_user", "o_project", "o_artStyle", "o_agentDeploy", "o_tasks", "o_prompt", "o_modelPrompt",
  "o_novel", "o_event", "o_eventChapter", "o_script", "o_image", "o_assets", "o_storyboard",
  "o_agentWorkData", "o_video", "o_videoTrack", "o_imageFlow",
];

/** Call after a trusted explicit import that includes rows with explicit IDs. */
export async function syncBaseIdentitySequences(knex: Knex, tables = baseIdentityTables): Promise<void> {
  for (const table of tables) {
    await knex.raw(
      `WITH max_id AS (SELECT MAX("id") AS value FROM "${table}")
       SELECT setval(pg_get_serial_sequence(?, 'id'), COALESCE((SELECT value FROM max_id), 1), (SELECT value IS NOT NULL FROM max_id))`,
      [`"${table}"`],
    );
  }
}

/** Idempotent base-schema migration for a fresh PostgreSQL database. */
export async function ensureBaseSchema(knex: Knex, forceInit: boolean = false): Promise<void> {
  ...
  const tables: TableSchema[] = [
    ...
    //提示词表
    {
      ...
      initData: async (knex) => {
        await knex("o_prompt").insert(commonPromptSeeds);
      },
    },
    //模型绑定提示词表
    {
    ...
    //事件-章节表
    {
      ...
      builder: (table) => {
        identityId(table);
        bigint(table, "eventId").references("id").inTable("o_event");
        bigint(table, "novelId").references("id").inTable("o_novel");
        table.primary(["id"]);
        table.unique(["id"]);
      },
    },
    ...
    //资产表
    {
      ...
      builder: (table) => {
        ...
        table.text("type");
        table.text("describe");
        bigint(table, "scriptId"); //剧本id
        bigint(table, "imageId").references("id").inTable("o_image");
        bigint(table, "assetsId");
        bigint(table, "projectId");
        bigint(table, "flowId"); //工作流id
        bigint(table, "startTime");
    ...
    //分镜
    {
      ...
      builder: (table) => {
        identityId(table);
        bigint(table, "scriptId");
        table.text("prompt");
        table.text("filePath");
        table.specificType("duration", "double precision");
        table.text("state");
        bigint(table, "trackId");
        table.text("reason");
        table.text("track");
    ...
    //flowData-剧本
    {
      ...
      builder: (table) => {
        identityId(table);
        bigint(table, "projectId");
        bigint(table, "episodesId");
        table.string("key"); //用户其他方式索引
        table.text("data");
        bigint(table, "createTime");
        bigint(table, "updateTime");
        table.primary(["id"]);
        table.unique(["id"]);
      },
    },
    ...
    // 视频轨道
    {
      ...
      builder: (table) => {
        identityId(table);
        bigint(table, "videoId");
        bigint(table, "projectId");
        bigint(table, "scriptId");
        table.text("state");
        table.text("reason");
        table.text("prompt");
        bigint(table, "selectVideoId");
        table.specificType("duration", "double precision");
        table.primary(["id"]);
        table.unique(["id"]);
      },
    ...
    },
    {
      ...
      builder: (table) => {
        identityId(table);
        bigint(table, "storyboardId").notNullable();
        bigint(table, "assetId").notNullable();
        // SQLite rowid was accidentally used as presentation order. This
        // generated id is an explicit, portable replacement.
        table.primary(["storyboardId", "assetId"]);
        table.unique(["storyboardId", "assetId"]);
        table.index(["storyboardId", "id"]);
      },
    },
    {
      name: "o_scriptAssets",
      builder: (table) => {
        bigint(table, "scriptId").notNullable();
        bigint(table, "assetId").notNullable();
    ...
    },
    {
      ...
      builder: (table) => {
        ...
        bigint(table, "createTime").notNullable();
        bigint(table, "updateTime").notNullable();
      ...
      initData: async (knex) => {
        ...
        // Empty seed embeddings keep schema creation independent of model execution.
    ...
    },
    {
      ...
      builder: (table) => {
        bigint(table, "assetsRoleId").notNullable();
        bigint(table, "assetsAudioId").notNullable();
  ...
  // A pre-PostgreSQL development schema can have this link table without an
  // portable ordering id. Additive migration preserves rows and avoids table rebuilds.
  if (await knex.schema.hasTable("o_assets2Storyboard")) {
    await knex.raw('ALTER TABLE "o_assets2Storyboard" ADD COLUMN IF NOT EXISTS "id" bigint GENERATED BY DEFAULT AS IDENTITY');
    await knex.raw('CREATE INDEX IF NOT EXISTS "o_assets2Storyboard_order_idx" ON "o_assets2Storyboard" ("storyboardId", "id")');
  }

  // The workspace stores serialized plans/scripts, not a short identifier.
  // Existing PostgreSQL installs inherited Knex string's 255-character cap.
  const workspaceData = await knex("o_agentWorkData").columnInfo("data");
  if (workspaceData.type === "character varying" || workspaceData.type === "varchar") {
    await knex.transaction(async (trx) => {
      await trx.raw("SET LOCAL lock_timeout = '5s'");
      await trx.raw('ALTER TABLE "o_agentWorkData" ALTER COLUMN "data" TYPE text');
    });
  }

  // Seed/import code may provide explicit IDs (notably the administrator row).
  // PostgreSQL identities do not advance for those inserts, so synchronize each
  // sequence without rebuilding or rewriting any existing table.
  await syncBaseIdentitySequences(knex, forceInit ? baseIdentityTables : [...createdTables].filter((table) => baseIdentityTables.includes(table)));
}

export default ensureBaseSchema;
