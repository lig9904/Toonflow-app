...
export async function prepareDerivedAssetImages(db: Knex, args: {
  projectId: number; scriptId: number; assetIds: number[]; concurrentCount?: number; runtime: ProductionImageRuntime;
}) {
...
export async function prepareStoryboardImages(db: Knex, args: {
  projectId: number; scriptId: number; storyboardIds: number[]; concurrentCount?: number; compulsory?: boolean; runtime: ProductionImageRuntime;
}) {
