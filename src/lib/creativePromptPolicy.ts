...
export function assetPromptSystem(visualManual: string, singleImage = false): string {
  ...
  if(singleImage)visualManual=visualManual.split(/\r?\n/).filter(line=>!/四视图|多视图|三视图|多视角|四宫格|多宫格|拼版|拼图|正面|侧面|背面/.test(line)).join("\n");
...
export function assertSingleAssetImage(prompt:string):void{
 const positive=prompt.replace(/(?:不|不要|禁止|避免|无需|非|无|杜绝|拒绝)[^，。；\n]*(?:四视图|多视图|三视图|四宫格|多宫格|拼图|拼版)[^，。；\n]*/g,"");
