...
import { acknowledgeVideoPromptReferences, captureVideoModeSelectionSnapshot, claimVideoModeSelectionForSubmission, ensureVideoModeIntentSchema, readVideoModeIntent, resolveStoredVideoMode, revalidateVideoModeSelection, saveVideoModeIntent, saveVideoReferences } from "../src/services/videoModeResolution";
...
test("project mode stays a legacy default while every track without an explicit receipt defaults to auto", options, async () => {
  ...
  try {
    ...
    assert.deepEqual(await readVideoModeIntent(fixture.db, { projectId, scriptId, trackId: oldTrack }), { trackId: oldTrack, modeIntent: "auto", references: [], referencesInitialized: false, promptReferenceRevision: 0, revision: 0, source: "default" });
    const [newTrack] = await insertRowsReturningIds(fixture.db, "o_videoTrack", { projectId, scriptId, prompt: "new human prompt" });
    assert.deepEqual(await readVideoModeIntent(fixture.db, { projectId, scriptId, trackId: newTrack }), { trackId: newTrack, modeIntent: "auto", references: [], referencesInitialized: false, promptReferenceRevision: 0, revision: 0, source: "default" });
