...
- `POST /builtinAgent/start`: `{agentType,projectId,scriptId?,prompt,idempotencyKey,limits?}` → `{run: BuiltinRunView,reused:boolean}`. Full shared types are in `contracts.ts`. Generation allowances default to zero; explicit user authorizations are included in the run limits and server checks.
